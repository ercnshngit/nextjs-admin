...
const authPassPaths = [
  ...
  { path: `/api/table/${PARAM}/update/${PARAM}`, method: "GET", hasParams: true },
];
...
function isPathHasAuthPass(path: string, request : any) : boolean {
  ...
  authPassPaths.forEach((authPassPath) => {
    if(authPassPath.hasParams){
      ...
      if (pathSplit.length === authPassPathSplit.length) { // pathlerin parçalanmıs hallerının eleman mıtkarı kontrol edılıyor
        let isSame = true;
        for (let i = 0; i < pathSplit.length; i++) { // pathlerin her bir elemanı kontrol ediliyor
          if (authPassPathSplit[i] === PARAM) { // eğer parametreyse kontrol edilmiyor VE BU SEKILDE o / x / arasındakı parametre degerıde pass gecılıyor
            continue;
          }
          if (pathSplit[i] !== authPassPathSplit[i]) { // eğer parametre değilse ve pathlerin elemanları eşit değilse false dönüyor
            isSame = false;
            break;
          }
        }
        if (isSame && authPassPath.method === request.method) { // eğer pathlerin elemanları eşitse ve parametre değilse true dönüyor
          isTrue = true;
        }
      }
    }
    if (path.includes(authPassPath.path)) { // eğer pathlerin parçalanmıs hallerının eleman mıtkarı eşit değilse ve pathlerin içerisinde authPassPath varsa true dönüyor
