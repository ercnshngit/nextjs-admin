...
export class BaseService{
    ...
    constructor(request?: any) {
        ...
        this.api_path = request == undefined ? "" : this.request.nextUrl.pathname;
    ...
    async permissionCheck(apiRoutePermissionInfo : any, isAuth : any) {
        try {
            if(apiRoutePermissionInfo.is_role){ // Role kontrolu yapılıyor
                if(await this.permissionService.isUserHasAPermission(isAuth.user,apiRoutePermissionInfo.id)){
                  return;
                }else{
                  throw new Response(JSON.stringify({ error: ErrorMessages.PERMISSION_ERROR() }), {
                    status: 403,
                  });
                }
              }else{
    ...
    async authAndPermissionCheck(apiRoutePermissionInfo : any) {
        try {
            ...
            if (!isAuth.status) {       // Eğer giriş yapılmamışsa hata döndürüyor
    ...
    async securiyCheck() {
        try {
            ...
            const apiRoutePermissionInfo = await this.permissionService.getApiRouteInfo(this.api_path, this.request);
            if(!apiRoutePermissionInfo){            // Eğer api route kaydı yoksa direkt girişe izin veriyor
                return;   
            }else{
