...
export class TagService {
    async getTag(id: number) {
        ...
        if (!tag) { return new Response(JSON.stringify({ message: ErrorMessages.NOT_FOUND_ERROR() })); }
        return new Response(JSON.stringify(tag));
    }

    async getTags() {
        const tags = await prisma.tag.findMany()
        if (tags.length < 1) { return new Response(JSON.stringify({ message: ErrorMessages.NOT_FOUND_ERROR() })); }
    ...
    async updateTag(id: number, data: TagDto) {
        try {
            ...
            if (!new_tag) { return new Response(JSON.stringify({ message: ErrorMessages.UPDATE_FAILED_ERROR() })); }
    ...
    async deleteTag(id: number) {
        try {
            ...
            if (!delete_tag) { return new Response(JSON.stringify({ message: ErrorMessages.DELETE_FAILED_ERROR() })); }
