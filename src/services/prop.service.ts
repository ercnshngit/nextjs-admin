...
export class PropService {
    ...
    async getProp(id: number) {
        ...
        if (!prop) { return new Response(JSON.stringify({ message: ErrorMessages.NOT_FOUND_ERROR() })); }
        return new Response(JSON.stringify(prop));
    }

    async getProps() {
        const props = await prisma.prop.findMany()
        if (props.length < 1) { return new Response(JSON.stringify({ message: ErrorMessages.NOT_FOUND_ERROR() })); }
    ...
    async updateProp(id: number, data: PropDto) {
        try {
            ...
            if (!new_prop) { return new Response(JSON.stringify({ message: ErrorMessages.UPDATE_FAILED_ERROR() })); }
    ...
    async deleteProp(id: number) {
        try {
            ...
            if (!delete_prop) { return new Response(JSON.stringify({ message: ErrorMessages.DELETE_FAILED_ERROR() })); }
